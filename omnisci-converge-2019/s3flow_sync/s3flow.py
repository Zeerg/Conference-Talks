...
def options_parse(argv=None):
    ...
    parser.add_argument(
        "--protocol", help="Protocol for Thrift",
        default="binary")
    parser.add_argument(
        "-s", "--host", help="OmniScidb server address",
        default="localhost"
    )
    parser.add_argument(
        "-p", "--port", help="OmniScidb server port",
        default="6274"
    )
    parser.add_argument(
        "-d", "--db", help="OmniScidb database name",
        default="omnisci"
    )
    parser.add_argument(
        "-u", "--user", help="OmniScidb user name",
        default="admin"
    )
    parser.add_argument(
        "-pass", "--password", help="Omniscidb password",
        default="HyperInteractive"
    )
    parser.add_argument(
        "-mmdb", "--maxmind_db", help="Specify the maxmindb file",
        default="None"
    )
    parser.add_argument(
        "-tt", "--transform_threads", help="Specify the transform ETL threads to spawn",
        default=1
    )
    parser.add_argument(
        "-t", "--table",
        help="OmniScidb table",
        default=None
    )
